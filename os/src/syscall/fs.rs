...
use crate::fs::{OpenFlags, ROOT_INODE, Stat, open_file};
...
pub fn sys_fstat(fd: usize, st: *mut Stat) -> isize {
    ...
    if let Some(file) = &fd_table[fd] {
        ...
        drop(inner);
        let stat = file.stat();
        let dst_chunks = translated_byte_buffer(token, st as *const u8, core::mem::size_of::<Stat>());
...
pub fn sys_linkat(_old_name: *const u8, _new_name: *const u8) -> isize {
    ...
    if let Some(old_inode) = ROOT_INODE.find(&old_name) {
        let inode_id = old_inode.id() as u32;
